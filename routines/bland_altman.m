...
data_mean = mean_nan([data1,data2],2);  % Mean of values from each instrument 
data_diff = data1 - data2;              % Difference between data from each instrument
md = mean_nan(data_diff);               % Mean of difference between instruments 
sd = std_nan(data_diff);                % Std dev of difference between instruments 

figure,plot(data_mean,data_diff,'ok','MarkerSize',10,'LineWidth',2)   % Bland Altman plot
