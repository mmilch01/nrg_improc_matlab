...
%
% where avw_hdr_make.m avw_hdr_make.m.bk avw_hdr_read.m avw_hdr_read.m.bk avw_hdr_write.m avw_hdr_write.m.bk avw_img_read.m avw_img_write.m avw_write.m blur3.m blur3_thin.m draw_line_3d.m draw_reper.m draw_shape.m draw_verts.m fill_layer.m mask_surf.m maskVol.m projectVol.m RectangularMesh.m renderVol.m save_vol.m saveVol.m _tmpfile follows the slice dimension and letters indicate +XYZ
...
msg = sprintf('...no input fileprefix - see help avw_img_readnn');
error(msg);
end
...
if fid < 0,
msg = sprintf('...cannot open file %s.imgnn',avw.fileprefix);
error(msg);
end

if verbose,
ver = '[$Revision: 1.3 $]';
fprintf('nAVW_IMG_READ [v%s]n',ver(12:16)); tic;
end
...
otherwise,
precision = 'float';
if verbose, fprintf('...precision undefined in header, using ''uchar''n'); end
end

% read the whole .img file into matlab (faster)
if verbose,
fprintf('...reading %s Analyze %s image format.n',machine,precision);
end
fseek(fid,0,'bof');
...
msg = [ '...unspecified avw.hdr.hist.orient, using default 0n',...
' (check image and try explicit IMGorient option).n'];
fprintf(msg);
...
if verbose, fprintf('...reading axial unflipped orientationn'); end
...
if verbose, fprintf('...reading coronal unflipped orientationn'); end
...
if verbose, fprintf('...reading sagittal unflipped orientationn'); end
...
if verbose, fprintf('...reading axial flipped (+Y from Anterior to Posterior)n'); end
...
if verbose, fprintf('...reading coronal flipped (+Z from Superior to Inferior)n'); end
...
if verbose, fprintf('...reading sagittal flipped (+Z from Superior to Inferior)n'); end
...
if verbose, t=toc; fprintf('...done (%5.2f sec).nn',t); end
