...
fprintf('AVW_HDR_WRITE [v%s]n',version(12:16)); tic;
end
...
if ~exist('avw','var'),
warning('...no input avw - calling avw_hdr_maken');
avw = avw_hdr_make;
elseif isempty(avw),
warning('...empty input avw - calling avw_hdr_maken');
avw = avw_hdr_make;
elseif ~isfield(avw,'hdr'),
warning('...empty input avw.hdr - calling avw_hdr_maken');
avw = avw_hdr_make;
end
if ~isequal(avw.hdr.hk.sizeof_hdr,348),
msg = sprintf('...avw.hdr.hk.sizeof_hdr must be 348!n');
error(msg);
end
...
error('...no input fileprefix - see help avw_hdr_writenn');
return;
end
end

if findstr('.hdr',fileprefix),
% fprintf('AVW_HDR_WRITE: Removing .hdr extension from ''%s''n',fileprefix);
...
% if ( currDim < minDim )
% % fprintf( 'Warning %s: Forcing %d dimensions in avw.hdr.dime.dimn', ...
% % mfilename, minDim );
...
if fid < 0,
msg = sprintf('Cannot write to file %s.hdrn',fileprefix);
error(msg);
else
if verbose, fprintf('...writing %s Analyze header.n',machine); end
...
msg = sprintf('...file size is not 348 bytes!n');
warning(msg);
end
